...
2. For each record it is provided:
...
Dataset1 
- "activity_labels.txt" files was used to decode y_test.txt and y_train.txt from numeric to character labels (WALKING, WALKING_UPSTAIRS, WALKING_DOWNSTAIRS, SITTING, STANDING, LAYING)
- "features.txt" was used to create column names for X_test.txt and X_train.txt
- "subject_test.txt" and "subject_train.txt" columns were added to X_test.txt and X_train.txt respectively
- Resulting from above two pieces (Test and Training) datasets were combined (merged) by rows
- From the all list of variables in the resulting dataset I removed all that are NOT mean or standard deviation measures (column with subject and actity labeld were also preserved). 

Dataset2
- For the second tidy dataset all column means were calculated per subject and per activity simultaneously. This was implemented by creation of an intermediate variable 
"new_col", obtained by concatenation of subject ID and activity label, resulting in a sting like "10-WALKING"
- The resulting dataset "mean_data2_final.txt" contains "subject", "activity", and mean and standard deviation variables from the Dataset1



APPENDIX 
(List of ALL variables in the RAW dataset)
1 tBodyAcc-mean()-X
2 tBodyAcc-mean()-Y
3 tBodyAcc-mean()-Z
4 tBodyAcc-std()-X
